...
- [Step 4: Install the mini SD card into the Raspberry Pi](#step-4-install-the-mini-SD-card-into-the-raspberry-pi)
...
* USB Ports: Pretty much any USB device can go here, we will use for keyboard and mouse
* Ethernet Port: Connection for a wireless network
...
* Display Connector: An optional display port, we will not use
...
*	Plug one end of the hdmi cable into the hdmi port on the pi (refer to step 2 if you forget where that is)
*	If you monitor has a hdmi port then plug the free end of the hdmi cable into your monitor
...
### Step 10: Updating the Raspberry Pi software
...
*	Open a terminal window by clicking the terminal icon at the top of the screen as show below
...
apt-get update
```
You will notice you get an error message, that is because installing or updating software requires administrative privileges.  Although the user _pi_ is an administrator the Raspberry Pi enforces the first principle of __Least Privilege__ which says a user should have no more privileges than what is needed to do their job.  Generally you do not need administrator privileges to use the raspberry pi so by default they are not provided.  We need to tell the pi we want to use administrative privileges by using the command _sudo_, think of this command as super user do.  By proceeding a command with the command sudo we are telling the system to execute that program as an administrator or super user.  If the user is not an approved administrator this command will fail.

```bash
sudo apt-get update
```

Apt-get is the Linux program used to install and update software.  The option update tells the operating system to download the most recent list of available software.  Now enter the following
...
Now that the software has been updated we are done with the Raspberry Pi for now and can turn it off.  We could do this through the GUI with the mouse but we already have a terminal window open, here’s a command to turn off the Raspberry Pi off from the command line
...
### Evaluation
Develop a Kahoot which asks:
* A Raspberry pi uses modularity because: _it uses separate components for many of its functions.  Some like the camera can be installed and removed as needed being replaced with any other similar camera module_
* The Linux command sudo demonstrates which cybersecurity first principle: _least privilege_
* Which component of the raspberry pi stores the operating system: _SD card_
